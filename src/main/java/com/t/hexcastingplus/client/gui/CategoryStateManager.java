...
public class CategoryStateManager {
    private static final String STATE_FILE = "hexcasting_patterns/category_state.json";
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();
    private static Map<String, Boolean> categoryStates = new HashMap<>();

    public static void load() {
        try {
            Path statePath = Minecraft.getInstance().gameDirectory.toPath().resolve(STATE_FILE);
